...
public class SysRoleConvertImpl implements SysRoleConvert {
    ...
    @Override
    public RoleVO clone(RoleVO source, RoleVO target) {
        if ( source == null ) {
            return null;
        }
        ...
        target.setId( source.getId() );
        if ( target.getDepartmentIds() != null ) {
            List<Long> list = source.getDepartmentIds();
            if ( list != null ) {
                target.getDepartmentIds().clear();
                target.getDepartmentIds().addAll( list );
            }
            else {
                target.setDepartmentIds( null );
            }
            ...
            List<Long> list = source.getDepartmentIds();
            if ( list != null ) {
                target.setDepartmentIds( new ArrayList<Long>( list ) );
            }
        }
        if ( target.getMenuIds() != null ) {
            Set<Long> set = source.getMenuIds();
            if ( set != null ) {
                target.getMenuIds().clear();
                target.getMenuIds().addAll( set );
            }
            else {
                target.setMenuIds( null );
            }
            ...
            Set<Long> set = source.getMenuIds();
            if ( set != null ) {
                target.setMenuIds( new HashSet<Long>( set ) );
            }
        }
        target.setName( source.getName() );
        target.setLevel( source.getLevel() );
